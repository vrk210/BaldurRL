...
def roll_d20(rng: Random) -> int:
    return rng.randint(1, 20)


def roll_damage(attacker: Character, rng: Random, critical: bool = False) -> int:
    """Roll damage, doubling dice (but not the bonus) on a critical hit."""
    dice_count = attacker.damage_dice_count * (2 if critical else 1)
    dice_total = sum(rng.randint(1, attacker.damage_die_size) for _ in range(dice_count))
    return max(0, dice_total + attacker.damage_bonus)


def resolve_attack(attacker: Character, defender: Character, rng: Random) -> AttackResult:
    """Resolve one attack and apply any damage to the defender's HP."""
    d20_roll = roll_d20(rng)
    critical = d20_roll == 20
    hit = critical or (d20_roll != 1 and d20_roll + attacker.attack_bonus >= defender.armor_class)
    if not hit:
        return AttackResult(d20_roll=d20_roll, hit=False, critical=False, damage=0)

    rolled_damage = roll_damage(attacker, rng, critical=critical)
    damage = min(defender.hp, rolled_damage)
    defender.hp -= damage
    return AttackResult(d20_roll=d20_roll, hit=True, critical=critical, damage=damage)


def use_fighter_attack(fighter: Fighter, goblin: Goblin, rng: Random) -> AttackResult:
...
def use_second_wind(fighter: Fighter, rng: Random) -> int:
